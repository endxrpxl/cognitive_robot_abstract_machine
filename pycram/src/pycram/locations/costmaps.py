...
class OccupancyCostmap(Costmap):
    ...
    def create_ray_mask_around_origin(self):
        ...
        xy = np.stack((row_grid, col_grid), axis=-1).reshape(
            -1, 2
        ) * self.resolution + np.array(origin_position[:2], dtype=float)

        # base height of the robot plus a safety offset
        base_height = self.robot_view.base.bounding_box.height + 0.1
        ray_starts = np.column_stack((xy, np.full(len(xy), base_height, dtype=float)))
