...
class CostmapLocation(Location):
    ...
    def _describe_target(target: Pose) -> str:
        ...
        return (
            f"Pose(x={float(pos[0]):.3f}, y={float(pos[1]):.3f}, z={float(pos[2]):.3f})"
        )
    ...
    def _allowed_xy_bounds(self) -> Optional[tuple[float, float, float, float]]:
        ...
        floor_bb = (
            floors[0]
            .root.collision.as_bounding_box_collection_in_frame(self.world.root)
            .bounding_box()
        )
        return (
    ...
    def _room_free_space_gcs(self) -> Optional[GraphOfConvexSets]:
        ...
        room_annotations = self.world.get_semantic_annotations_by_type(Room)
        if room_annotations:
    ...
    def setup_costmaps(self, target: Pose, visible: bool, reachable: bool) -> Costmap:
        ...
        room_layout = (
            None
            if self.allowed_area_points is not None
            else self._room_search_space_and_origin()
        )
        ...
        if not self.validate_reachability:
            self._last_costmap_source = (
                "occupancy_room" if room_based_map else "occupancy"
            )
    ...
    def __iter__(self) -> Iterator[Pose]:
        ...
        for pose_candidate in final_map:
            ...
            if (
                self.allowed_area_points is not None
                and not self._is_within_room_bounds(pose_candidate)
            ):
                continue
        ...
        for pose_candidate in final_map:
            ...
            for arm_chain in test_robot.get_arms():
                ...
                is_reachable = pose_sequence_reachability_validator(
                    ...
                    test_robot,
                    test_world,
                    use_fullbody_ik=test_robot.mobile_base.full_body_controlled,
                )
                if is_reachable:
                    yield pose_candidate
